...
func randomtext() (randomtext string) {
	...
	radomtext[2] = "There are two ways of constructing a software design. One way is to make it so simple that \n there are obviously no deficiencies and the other is to make it so \n complicated that there are no obvious deficiencies."
...
func main() {
	http.HandleFunc("/echo", func(w http.ResponseWriter, r *http.Request) {
		...
		newText1 := randomtext()
		conn.WriteMessage(websocket.TextMessage, []byte(newText1))
